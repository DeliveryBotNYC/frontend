...
const CustomApi = ({ stateChanger, state }) => {
    ...
    useEffect(() => {
        const wasPopupOpen = contextValue?.editApi ||
    ...
    const isAnyPopupOpen = contextValue?.editApi ||
        contextValue?.generateAPI ||
        contextValue?.showGeneratedApiKey;
    return (_jsxs("div", { children: [_jsx(IntegrationCard, { icon: ApiIcon, title: "Custom API", isConnected: accountData.active, isLoading: isLoading || isConfiguring, onConfigure: handleConfigure, lastUsed: accountData.last_used, subtext: accountData.requests + " requests", error: error ? "Failed to load API data" : undefined }), isAnyPopupOpen && _jsx(BlackOverlay, { closeFunc: closePopup }), _jsx(ApiPopup, {})] }));
};
