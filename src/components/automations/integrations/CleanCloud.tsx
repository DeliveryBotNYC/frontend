...
interface CleanCloudProps {
  stateChanger: (data: any) => void;
  state?: any;
}

const CleanCloud = ({ stateChanger, state }: CleanCloudProps) => {
  ...
  useEffect(() => {
    const wasPopupOpen =
      contextValue?.cleanCloud || contextValue?.cleanCloudUpdate;
  ...
  return (
    <div>
      <IntegrationCard
        ...
        error={error ? "Failed to load CleanCloud data" : undefined}
      />
