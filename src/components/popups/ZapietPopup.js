...
const ZapietPopup = () => {
    ...
    const getCurrentMode = () => {
        if (contextValue?.editZapiet)
            return "edit";
        if (contextValue?.generateZapiet)
            return "generate";
        if (contextValue?.showGeneratedZapietKey)
            return "generated";
        return null;
    };
    ...
    const handleEditToGenerate = () => {
        if (window.confirm("Are you sure you want to update your Zapiet key? This action cannot be undone and will invalidate your current key.")) {
    ...
    const handleDelete = () => {
        if (window.confirm("Are you sure you want to delete this Zapiet key? This action cannot be undone.")) {
    ...
    const renderContent = () => {
        switch (currentMode) {
            case "edit":
                return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "w-full mt-4", children: [_jsx("h3", { className: "text-black font-semibold text-lg", children: "Zapiet" }), _jsx("p", { className: "text-sm text-themeDarkGray mt-1", children: "Zapiet allows you to integrate your custom application and website with Delivery Bot's courier network." }), _jsx(Link, { to: "https://www.google.com", target: "_blank", children: _jsx("p", { className: "text-themeDarkBlue text-sm mt-3", children: "Documentation: www.dbx.delivery/zapiet" }) })] }), isLoading && (_jsx("div", { className: "mt-5 text-center", children: _jsx("p", { className: "text-themeDarkGray", children: "Loading Zapiet data..." }) })), error && (_jsxs("div", { className: "mt-5 text-center", children: [_jsx("p", { className: "text-red-500", children: "Failed to load Zapiet data" }), _jsx("button", { onClick: () => refetch(), className: "text-themeDarkBlue text-sm mt-2 underline", children: "Retry" })] })), status === "success" && data && (_jsxs("div", { className: "mt-5", children: [_jsx(ApiStats, { label: "Date created:", value: formatDate(accountData.created_at) }), _jsx(ApiStats, { label: "Last used:", value: formatDate(accountData.last_used) }), _jsx(ApiStats, { label: "Number of requests", value: accountData.requests.toString() })] })), _jsxs("div", { className: "space-y-3 mt-3", children: [_jsx("button", { onClick: handleEditToGenerate, disabled: updateZapietMutation.isPending, className: "w-full bg-themeLightOrangeTwo py-2.5 text-white font-semibold rounded-lg shadow-btnShadow hover:scale-95 duration-200 disabled:opacity-50", children: updateZapietMutation.isPending ? "Updating..." : "Update" }), _jsx("button", { onClick: handleDelete, disabled: deleteZapietMutation.isPending, className: "w-full bg-red-500 py-2.5 text-white font-semibold rounded-lg shadow-btnShadow hover:scale-95 duration-200 disabled:opacity-50", children: deleteZapietMutation.isPending ? "Deleting..." : "Delete" })] })] }));
            case "generate":
                return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "w-full mt-4", children: [_jsx("h3", { className: "text-black font-semibold text-lg", children: "Zapiet" }), _jsx("p", { className: "text-sm text-themeDarkGray mt-1", children: "Zapiet allows you to integrate your custom application and website with Delivery Bot's courier network." }), _jsx(Link, { to: "https://www.google.com", target: "_blank", children: _jsx("p", { className: "text-themeDarkBlue text-sm mt-3", children: "Documentation: www.dbx.delivery/zapiet" }) })] }), _jsx("button", { onClick: handleGenerateToGenerated, className: "w-full bg-themeLightOrangeTwo mt-8 py-2.5 text-white font-semibold rounded-lg shadow-btnShadow hover:scale-95 duration-200", children: "Generate key" })] }));
            case "generated":
                return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "w-full mt-4", children: [_jsx("h3", { className: "text-black font-semibold text-lg", children: "Keep your key safe!" }), _jsx("p", { className: "text-sm text-themeDarkGray mt-1", children: "Keep your key safe and store it in a secure place. You won't be able to see it again." }), _jsxs("div", { className: "w-full mt-6", children: [_jsx("p", { className: "text-xs text-themeDarkGray", children: "Zapiet key" }), _jsxs("div", { className: "border-b border-b-themeLightGray pb-[2px] flex items-center justify-between gap-2.5", children: [_jsx("p", { className: "text-black w-full overflow-auto", children: generateZapietMutation.isPending
                                                        ? "Generating..."
                                                        : generateZapietMutation.data?.data?.data?.token ||
                                                            generateZapietMutation.data?.data?.token ||
                                                            "Failed to generate" }), (generateZapietMutation.data?.data?.data?.token ||
                                                    generateZapietMutation.data?.data?.token) && (_jsx("div", { onClick: () => handleCopy(generateZapietMutation.data.data.data.token ||
                                                        generateZapietMutation.data.data.token), children: _jsx("img", { src: CopyIcon, alt: "copyBtn", className: "cursor-pointer" }) }))] })] })] }), _jsx("button", { onClick: closePopup, className: "w-full bg-themeGreen mt-8 py-2.5 text-white font-semibold rounded-lg shadow-btnShadow hover:scale-95 duration-200", children: "Continue" })] }));
            default:
                return null;
        }
    };
    return (_jsxs("div", { className: `w-[90%] max-w-[400px] bg-white rounded-xl p-6 fixed left-1/2 top-1/2 ${currentMode
            ...
            : contextValue?.hidePopupStyles} -translate-x-1/2 -translate-y-1/2 z-[99999] duration-300`, children: [_jsxs("div", { className: "flex items-start justify-between gap-2", children: [_jsx("img", { src: ZapietIcon, alt: "zapiet-icon", width: 60 }), _jsx("div", { onClick: closePopup, children: _jsx("img", { src: CloseIcon, alt: "close-icon", className: "cursor-pointer" }) })] }), renderContent()] }));
};
