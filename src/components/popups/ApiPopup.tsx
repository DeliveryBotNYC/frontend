...
const ApiPopup = () => {
  ...
  const getCurrentMode = (): PopupMode | null => {
    if (contextValue?.editApi) return "edit";
    if (contextValue?.generateAPI) return "generate";
    if (contextValue?.showGeneratedApiKey) return "generated";
    return null;
  };
  ...
  const handleEditToGenerate = () => {
    if (
      window.confirm(
        "Are you sure you want to update your API key? This action cannot be undone and will invalidate your current key."
      )
    ) {
          ...
          <>
            <div className="w-full mt-4">
              ...
              <p className="text-sm text-themeDarkGray mt-1">
                API allows you to integrate your custom application and website
                with Delivery Bot's courier network.
              </p>
            ...
            {isLoading && (
              <div className="mt-5 text-center">
                <p className="text-themeDarkGray">Loading API data...</p>
              </div>
            )}
            ...
            <div className="space-y-3 mt-3">
              ...
              >
                {deleteApiMutation.isPending ? "Deleting..." : "Delete"}
              </button>
            </div>
          </>
        );
