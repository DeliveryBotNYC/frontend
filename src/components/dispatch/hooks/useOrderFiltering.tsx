...
export const useOrderFiltering = (
  ...
  const [searchExpandedStops, setSearchExpandedStops] = useState(
    new Set<string>()
  );
  ...
  const { filteredStops, hasSearchResults } = useMemo(() => {
    ...
    orderedItems.forEach((stop) => {
      ...
      if (stop.pickup?.orders) {
        const matchingPickupOrders = stop.pickup.orders.filter((order: any) => {
      ...
      if (stop.deliver?.orders) {
        const matchingDeliveryOrders = stop.deliver.orders.filter(
          (order: any) => {
            // Check order ID (exact match, case insensitive)
            const idMatch = order.order_id?.toLowerCase().includes(searchLower);
  ...
  useEffect(() => {
    if (searchTerm.trim() && filteredStops.length > 0) {
      const stopIds = new Set(
        filteredStops.map((stop) => `${stop.customer_id}-${stop.o_order}`)
      );
