...
} from "../utils/orderConstraints"; // ADD canExtractOrderFromStop
...
export const useOrderOperations = (
      ...
      const newOrderedItems = orderedItems
        .map((stop) => {
          ...
          if (updatedStop.pickup?.orders) {
            const filteredPickupOrders = updatedStop.pickup.orders.filter(
          ...
          if (updatedStop.deliver?.orders) {
            const filteredDeliveryOrders = updatedStop.deliver.orders.filter(
              (order: any) => order.order_id !== orderId
            );
            updatedStop.deliver = {
              ...updatedStop.deliver,
        ...
        .filter((stop) => {
          const totalOrders =
            (stop.pickup?.count || 0) + (stop.deliver?.count || 0);
  ...
  const mergeOrderIntoStop = async (
    order: any,
    orderType: string,
    ...
    console.log(
      `Attempting to merge order ${order.order_id} from stop ${
        sourceStopIndex + 1
      } to stop ${targetStopIndex + 1}`
    );
    ...
    if (targetStop.customer_id !== orderCustomerId) {
      console.log(
        `Cannot merge: Customer ID mismatch. Target: ${targetStop.customer_id}, Order ${orderType}: ${orderCustomerId}`
      );
      return false;
    }
    ...
    const orderExists =
      orderType === "pickup"
        ? newTargetStop.pickup?.orders?.some(
            (o: any) => o.order_id === order.order_id
          )
        : newTargetStop.deliver?.orders?.some(
            (o: any) => o.order_id === order.order_id
          );
    ...
    if (orderType === "pickup" && newSourceStop.pickup?.orders) {
      newSourceStop.pickup.orders = newSourceStop.pickup.orders.filter(
        (o: any) => o.order_id !== order.order_id
      );
      newSourceStop.pickup.count = newSourceStop.pickup.orders.length;
    } else if (orderType === "delivery" && newSourceStop.deliver?.orders) {
      newSourceStop.deliver.orders = newSourceStop.deliver.orders.filter(
        (o: any) => o.order_id !== order.order_id
      );
    ...
    const totalSourceOrders =
      (newSourceStop.pickup?.count || 0) + (newSourceStop.deliver?.count || 0);
    ...
    console.log(
      `Successfully merged order ${order.order_id} into stop ${newTargetStop.o_order}`
    );
    return true;
  };

  // ENHANCED: Function to create a new stop from an extracted order
  const createNewStopFromOrder = async (
    extractedOrder: any,
    orderType: string,
    sourceStopIndex: number,
    targetIndex: number
    ...
    console.log(
      `Creating new stop from order ${extractedOrder.order_id} at position ${
        targetIndex + 1
      }`
    );
    ...
    // ENHANCED: Use specific order extraction validation
    if (
      !canExtractOrderFromStop(orderedItems, extractedOrder.order_id, orderType)
    ) {
    ...
    if (orderType === "pickup" && sourceStop.pickup?.orders) {
      sourceStop.pickup.orders = sourceStop.pickup.orders.filter(
        (order: any) => order.order_id !== extractedOrder.order_id
      );
      sourceStop.pickup.count = sourceStop.pickup.orders.length;
    } else if (orderType === "delivery" && sourceStop.deliver?.orders) {
      sourceStop.deliver.orders = sourceStop.deliver.orders.filter(
        (order: any) => order.order_id !== extractedOrder.order_id
      );
    ...
    // Remove empty source stop
    const totalSourceOrders =
      (sourceStop.pickup?.count || 0) + (sourceStop.deliver?.count || 0);
    if (totalSourceOrders === 0) {
      const adjustedSourceIndex =
        sourceStopIndex > targetIndex ? sourceStopIndex + 1 : sourceStopIndex;
      console.log(
        `Removing empty source stop at adjusted index ${adjustedSourceIndex}`
      );
    ...
    console.log(
      `Successfully created new stop for order ${extractedOrder.order_id} at position ${newStop.o_order}`
    );
    return true;
  };

  // Function to handle stop reordering
  const handleStopReorder = async (
    draggedIndex: number,
    targetIndex: number
  ) => {
    const newOrderedItems = [...orderedItems];
