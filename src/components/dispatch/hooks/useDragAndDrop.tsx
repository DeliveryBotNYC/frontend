...
export const useDragAndDrop = (
  orderedItems: any[],
  onStopReorder: (draggedIndex: number, targetIndex: number) => void,
  onOrderMerge: (
    order: any,
    orderType: string,
  ...
  onOrderCreateNewStop: (
    order: any,
    orderType: string,
    sourceStopIndex: number,
    targetIndex: number
  ) => void,
  findStopIndexByOrder: (orderId: string) => number,
  isDropOnSameStop: (
    order: any,
    orderType: string,
    targetIndex: number
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    if (draggedOrder) {
      ...
      if (
        isDropOnSameStop(draggedOrder.order, draggedOrder.orderType, dropIndex)
      ) {
      ...
      if (targetStop) {
        ...
        const orderAlreadyInTarget =
          (draggedOrder.orderType === "pickup" &&
            targetStop.pickup?.orders?.some(
              (o: any) => o.order_id === draggedOrder.order.order_id
            )) ||
          (draggedOrder.orderType === "delivery" &&
            targetStop.deliver?.orders?.some(
              (o: any) => o.order_id === draggedOrder.order.order_id
            ));

        if (orderAlreadyInTarget) {
          console.log(
            `Order ${draggedOrder.order.order_id} already exists in target stop, forcing new stop creation`
          );
        ...
        console.log(
          `Merge decision for ${draggedOrder.orderType}: orderCustomerId=${orderCustomerId}, targetCustomerId=${targetStop.customer_id}, orderAlreadyInTarget=${orderAlreadyInTarget}, shouldMerge=${shouldMerge}`
        );

        if (!shouldMerge) {
          console.log(
            "Different customers or duplicate order - will create new stop"
          );
        }
      } else {
      ...
      if (shouldMerge) {
        ...
        console.log(
          "Merging order into existing stop (same customer, no duplicate)"
        );
        await onOrderMerge(
          draggedOrder.order,
          draggedOrder.orderType,
  ...
  const handleContainerDragOver = (e: React.DragEvent) => {
    ...
    const allDraggableItems =
      e.currentTarget.querySelectorAll("[data-stop-item]");
    ...
    if (draggedOrder) {
      ...
      if (sourceStopIndex !== -1) {
        ...
        onOrderCreateNewStop(
          ...
          draggedOrder.orderType,
          sourceStopIndex,
          orderedItems.length // Add at the end
        );
      }
      setDraggedOrder(null);
    }
  ...
  const handleOrderDragStart = (e: CustomEvent) => {
