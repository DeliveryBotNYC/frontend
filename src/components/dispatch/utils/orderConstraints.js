...
export const findStopIndexByOrder = (orderedItems, orderId) => {
    return orderedItems.findIndex((stop) => {
        const hasPickupOrder = stop.pickup?.orders?.some((order) => order.order_id === orderId);
        const hasDeliveryOrder = stop.deliver?.orders?.some((order) => order.order_id === orderId);
        return hasPickupOrder || hasDeliveryOrder;
    });
};
// Helper function to find pickup stop for a delivery order
export const findPickupStopIndex = (orderedItems, orderId) => {
    return orderedItems.findIndex((stop) => {
        return stop.pickup?.orders?.some((order) => order.order_id === orderId);
    });
};
// Helper function to find delivery stop for a pickup order
export const findDeliveryStopIndex = (orderedItems, orderId) => {
    return orderedItems.findIndex((stop) => {
        return stop.deliver?.orders?.some((order) => order.order_id === orderId);
    });
};
// Helper function to check if order is being dropped on the same stop
export const isDropOnSameStop = (orderedItems, order, orderType, targetStopIndex) => {
...
export const getOrderCustomerId = (order, orderType) => {
...
export const validatePickupDeliveryConstraints = (orderedItems, orderId, orderType, targetStopIndex) => {
    if (orderType === "pickup") {
        ...
        if (deliveryStopIndex !== -1 && targetStopIndex >= deliveryStopIndex) {
            console.log(`Cannot move pickup: Delivery is at stop ${deliveryStopIndex + 1}, pickup would be at ${targetStopIndex + 1}`);
            return false;
        }
    }
        ...
        if (pickupStopIndex !== -1 && targetStopIndex <= pickupStopIndex) {
            console.log(`Cannot move delivery: Pickup is at stop ${pickupStopIndex + 1}, delivery would be at ${targetStopIndex + 1}`);
            return false;
        }
    }
    return true;
...
export const validateCustomerConstraints = (orderedItems, order, orderType, targetStopIndex, forceMerge = false // NEW: Add explicit merge flag
) => {
    ...
    if (!canMerge) {
        console.log(`Customer ID mismatch for forced merge: Target stop customer_id=${targetStop.customer_id}, Order customer_id=${orderCustomerId}`);
    }
    return canMerge;
};
// FIXED: Validate all pickup/delivery constraints when merging orders into existing stops
export const validateOrderMergeConstraints = (orderedItems, order, orderType, targetStopIndex) => {
    // Check the basic pickup/delivery constraint for the order being moved
    if (!validatePickupDeliveryConstraints(orderedItems, order.order_id, orderType, targetStopIndex)) {
        return false;
    }
    ...
    const targetStop = orderedItems[targetStopIndex];
    if (!targetStop)
        return true; // Creating new stop, basic validation is enough
    ...
    if (targetStop.pickup?.orders) {
        for (const pickupOrder of targetStop.pickup.orders) {
            const deliveryStopIndex = findDeliveryStopIndex(orderedItems, pickupOrder.order_id);
            if (deliveryStopIndex !== -1 && targetStopIndex >= deliveryStopIndex) {
                console.log(`Cannot merge: Pickup order ${pickupOrder.order_id} in target stop would be after/same as its delivery at stop ${deliveryStopIndex + 1}`);
                return false;
            }
        }
    }
    ...
    if (targetStop.deliver?.orders) {
        for (const deliveryOrder of targetStop.deliver.orders) {
            const pickupStopIndex = findPickupStopIndex(orderedItems, deliveryOrder.order_id);
            if (pickupStopIndex !== -1 && targetStopIndex <= pickupStopIndex) {
                console.log(`Cannot merge: Delivery order ${deliveryOrder.order_id} in target stop would be before/same as its pickup at stop ${pickupStopIndex + 1}`);
                return false;
            }
        }
    }
...
export const validateStopMoveConstraints = (orderedItems, stopIndex, targetIndex) => {
    ...
    if (stop.pickup?.orders) {
        for (const pickupOrder of stop.pickup.orders) {
            ...
            if (deliveryStopIndex !== -1 && targetIndex >= deliveryStopIndex) {
                console.log(`Cannot move stop: Contains pickup order ${pickupOrder.order_id} that would be after its delivery`);
                return false;
            }
        }
    }
    ...
    if (stop.deliver?.orders) {
        for (const deliveryOrder of stop.deliver.orders) {
            ...
            if (pickupStopIndex !== -1 && targetIndex <= pickupStopIndex) {
                console.log(`Cannot move stop: Contains delivery order ${deliveryOrder.order_id} that would be before its pickup`);
                return false;
            }
        }
    }
...
export const canExtractOrderFromStop = (orderedItems, orderId, orderType) => {
    // Find the order in the stops
    for (const stop of orderedItems) {
        // Check pickup orders
        if (orderType === "pickup" && stop.pickup?.orders) {
            const pickupOrder = stop.pickup.orders.find((order) => order.order_id === orderId);
            if (pickupOrder) {
                // Can extract if the specific order is not locked (regardless of stop status)
                const isOrderSpecificallyLocked = pickupOrder.locked ||
        ...
        if (orderType === "delivery" && stop.deliver?.orders) {
            const deliveryOrder = stop.deliver.orders.find((order) => order.order_id === orderId);
            if (deliveryOrder) {
                // Can extract if the specific order is not locked (regardless of stop status)
                const isOrderSpecificallyLocked = deliveryOrder.locked ||
