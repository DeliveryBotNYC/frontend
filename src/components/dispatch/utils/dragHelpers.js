...
import { isDropOnSameStop, validateOrderMergeConstraints, validateStopMoveConstraints, getOrderCustomerId, canExtractOrderFromStop, } from "./orderConstraints";
import { isStopLocked, canMoveStopToPosition, } from "./stopHelpers";
// FIXED: Enhanced helper function to check if a drop is valid
export const isDropValid = (orderedItems, draggedData, targetIndex) => {
    ...
    if (draggedOrder) {
        ...
        if (!validateStopMoveConstraints(orderedItems, stopIndex, targetIndex)) {
            console.log("Drop invalid: Stop move would violate pickup/delivery constraints");
            return false;
        }
...
export const canMergeWithCustomer = (draggedOrder, targetStop) => {
...
export const createStopDragImage = (item) => {
    ...
    <div class="text-xs text-gray-500">${item.address?.street_address_1 || "No address"}</div>
    <div class="text-xs text-blue-600 mt-1">
      ${item.pickup?.count || 0} pickup(s), ${item.deliver?.count || 0} delivery(s)
    </div>
  `;
...
// Clean up drag image
export const cleanupDragImage = (dragElement) => {
    setTimeout(() => {
...
export const isOverEmptySpace = (e, allDraggableItems) => {
