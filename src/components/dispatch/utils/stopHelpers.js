...
export const isStopLocked = (stop) => {
    if (!stop)
        return false;
    ...
    const hasLockedPickup = stop.pickup?.locked ||
        stop.pickup?.orders?.some((order) => order.locked ||
    ...
    const hasLockedDelivery = stop.deliver?.locked ||
        stop.deliver?.orders?.some((order) => order.locked ||
            order.status === "completed" ||
...
export const isOrderLocked = (orderedItems, orderId) => {
...
export const canMoveStopToPosition = (orderedItems, stopIndex, targetPosition) => {
    ...
    console.log(`Checking if stop ${stopIndex + 1} can move to position ${targetPosition + 1}`);
    ...
    if (targetPosition >= orderedItems.length) {
        ...
        const adjustedTargetPosition = targetPosition > stopIndex ? targetPosition - 1 : targetPosition;
    ...
    if (stop.pickup?.orders) {
        for (const pickupOrder of stop.pickup.orders) {
            const deliveryStopIndex = findDeliveryStopIndex(simulatedArray, pickupOrder.order_id);
            if (deliveryStopIndex !== -1 && finalStopPosition >= deliveryStopIndex) {
                console.log(`Cannot move stop: Pickup order ${pickupOrder.order_id} would be at position ${finalStopPosition + 1} but its delivery would be at ${deliveryStopIndex + 1}`);
                return false;
            }
        }
    }
    ...
    if (stop.deliver?.orders) {
        for (const deliveryOrder of stop.deliver.orders) {
            const pickupStopIndex = findPickupStopIndex(simulatedArray, deliveryOrder.order_id);
            if (pickupStopIndex !== -1 && finalStopPosition <= pickupStopIndex) {
                console.log(`Cannot move stop: Delivery order ${deliveryOrder.order_id} would be at position ${finalStopPosition + 1} but its pickup would be at ${pickupStopIndex + 1}`);
                return false;
            }
        }
    }
