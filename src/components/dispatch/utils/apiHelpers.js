...
export const updateStopOrdersDirectly = async (apiStops, routeId, url, config) => {
    try {
        ...
        const response = await axios.patch(`${url}/route/${routeId}/stops`, requestBody, config);
...
export const updateOrderStop = async (orderId, orderType, newOOrder, url, config) => {
    try {
...
export const updateAllAffectedOrders = async (newOrderedItems, url, config) => {
    ...
    newOrderedItems.forEach((stop, stopIndex) => {
        ...
        if (stop.pickup?.orders) {
            stop.pickup.orders.forEach((order) => {
                updatePromises.push(updateOrderStop(order.order_id, "pickup", newOOrder, url, config));
            });
        }
        // Update all delivery orders in this stop
        if (stop.deliver?.orders) {
            stop.deliver.orders.forEach((order) => {
                updatePromises.push(updateOrderStop(order.order_id, "delivery", newOOrder, url, config));
            });
        }
    });
    try {
...
export const removeOrderFromRoute = async (orderId, url) => {
    try {
