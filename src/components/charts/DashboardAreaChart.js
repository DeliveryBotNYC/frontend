...
const DashboardAreaChart = ({ item }) => {
    ...
    const chartData = item.chart?.map((point) => point.orders) || [];
    const chartLabels = item.chart?.map((point) => point.date) || [];
    ...
    const options2 = {
        ...
        ">
          ...
          <div style="color: #ccc;">${orders} order${orders !== 1 ? "s" : ""}</div>
        </div>`;
            },
        },
        fill: {
    ...
    return (_jsx("div", { className: "relative", children: _jsx(ReactApexChart, { options: options2, series: series, type: "area", height: 60 }) }));
};
