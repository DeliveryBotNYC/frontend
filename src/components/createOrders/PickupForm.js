...
// Utils
import { formatPhoneWithPrefix, enforcePhoneFormat, isCompleted, initialState, isEmpty, parseClipboardText, formatName, } from "../reusable/functions";
...
const normalizePickupData = (data) => {
    ...
    return {
        ...
        address: {
            formatted: data.address?.formatted || data.address?.formatted_address || "",
            street_address_1: data.address?.street_address_1 || data.address?.street || "",
...
const createApiService = (config) => ({
    ...
    validateAddress: (address) => axios.get(`${url}/address/validate?street=${encodeURI(address.street_address_1)}&zip=${encodeURI(address.zip)}`, config),
});
const PickupForm = memo(({ data, stateChanger, state }) => {
    ...
    const updatePickupData = useCallback((updates) => {
    ...
    const updatePickupField = useCallback((field, value) => {
    ...
    const handlePictureVerificationChange = useCallback((e) => {
        updatePickupData({
            required_verification: {
    ...
    const renderPhoneOnlyForm = () => (_jsxs("div", { className: "w-full grid grid-cols-1 gap-2.5 px-5 pb-3", children: [_jsx(FormInput, { label: "Phone", id: "pickup_phone", type: "tel", prefix: "+1", required: true, value: state?.pickup?.phone || "", onChange: handlePhoneChange, onKeyUp: formatPhoneWithPrefix, onKeyDown: enforcePhoneFormat, isPhone: true, autoComplete: "tel", placeholder: "" }), _jsx("div", { className: "flex items-center justify-between", children: _jsx("img", { src: clipart, alt: "pickup-clipart" }) })] }));
    const renderFullForm = () => (_jsxs("div", { className: "w-full grid grid-cols-2 gap-2.5 px-5 pb-3", children: [_jsx(FormInput, { label: "Phone", id: "pickup_phone", type: "tel", prefix: "+1", required: true, disabled: !permissions.canEdit, value: state?.pickup?.phone || "", onChange: handlePhoneChange, onKeyUp: formatPhoneWithPrefix, onKeyDown: enforcePhoneFormat, isPhone: true, autoComplete: "tel" }), _jsx(FormInput, { ref: nameInputRef, label: "Name", id: "pickup_name", required: true, disabled: !permissions.canEdit, value: state?.pickup?.name || "", onChange: handleNameChange, capitalize: true }), _jsx("div", { className: `w-full ${!state?.pickup?.address?.lat ? "col-span-2" : ""}`, children: _jsx(AddressAutocomplete, { label: "Address", id: "pickup_address", required: true, disabled: !permissions.canEditAddress, value: state?.pickup?.address, onChange: handleAddressChange, error: addressError, placeholder: "" }) }), state?.pickup?.address?.lat && (_jsxs("div", { className: "w-full flex items-center justify-between gap-2.5", children: [_jsx(FormInput, { ref: aptInputRef, label: "Apt", id: "pickup_apt", disabled: !permissions.canEdit, value: state?.pickup?.apt || "", onChange: (e) => updatePickupField("apt", e.target.value) }), _jsx(FormInput, { label: "Access code", id: "pickup_access_code", disabled: !permissions.canEdit, value: state?.pickup?.access_code || "", onChange: (e) => updatePickupField("access_code", e.target.value) })] })), _jsx("div", { className: "w-full col-span-2", children: _jsx(FormTextarea, { label: "Courier note", id: "pickup_note", disabled: !permissions.canEdit, value: state?.pickup?.note || "", onChange: (e) => updatePickupField("note", e.target.value), maxLength: 100, showCharacterCount: true, rows: 1, resizable: true }) }), _jsx("div", { className: "w-full", children: _jsx(FormCheckbox, { label: "Proof of pickup", id: "pickup_picture", value: "Picture", disabled: !permissions.canEdit, checked: state?.pickup?.required_verification?.picture || false, onChange: handlePictureVerificationChange }) })] }));
    return (_jsxs("div", { className: "w-full bg-white rounded-2xl my-5", children: [_jsxs("div", { className: "py-5 px-2.5 flex items-center justify-between gap-2.5", children: [_jsxs("div", { className: "flex items-center gap-2.5", children: [_jsx("img", { src: validation.isFormCompleted ? PickupIconCompleted : PickupIconToDo, alt: "", "aria-hidden": "true" }), _jsx("h2", { className: "text-2xl text-black font-bold heading", children: "Pickup" })] }), renderHeaderActions()] }), !validation.isPhoneValid && state?.status === "new_order"
                ? renderPhoneOnlyForm()
                : renderFullForm()] }));
});
