    ...
    queryFn: async () => {
      return axios
        ...
        .catch((err) => {
          setError({
            message:
              err.response?.data?.message || "Failed to load orientation data",
          });
          throw err;
        });
    },
    ...
    <div className="pt-24 py-1.5 px-4 flex flex-col gap-4 text-white bg-[#404954] w-full h-full">
      {error?.message ? (
        ...
        <>
          ...
          {items.map((item, index) =>
            item.status === "to_do" ? (
              <div
                ...
                className={`flex items-center justify-between py-3 px-themePadding rounded-[20px] bg-[#A0A4AA] bg-opacity-50 text-black transition-all ${
                  isComponentLoading
              ...
              <div
                key={index}
                className="flex items-center justify-between py-3 px-themePadding rounded-[20px] bg-[#A0A4AA] bg-opacity-50 text-black"
              >
                <div>
              ...
              <div
                key={index}
                className="flex items-center justify-between py-3 px-themePadding rounded-[20px] bg-[#DBE8A8] bg-opacity-50 text-black"
              >
                <div>
          ...
          {/* Show loading indicator at bottom when mutating */}
          {addTodoMutation.isPending && (
            <div className="flex items-center justify-center py-2 text-sm text-gray-300">
