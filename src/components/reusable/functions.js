...
export const stadia = "";
export const mapStyle = "https://tiles.stadiamaps.com/tiles/alidade_smooth/{z}/{x}/{y}{r}.png?api_key=";
export function isCompleted(input) {
    return {
        pickup: !/^\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}$/.test(input.pickup.phone) ||
            input.pickup.name === "" ||
            ...
            ? false
            : true,
        delivery: !/^\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}$/.test(input.delivery.phone) ||
            input.delivery.name === "" ||
            ...
            ? false
            : true,
        timeframe: input.timeframe.service === "" ||
...
export const parseClipboardText = (text) => {
    ...
    try {
        ...
        // Address parsing
        const addressParts = parts.slice(1, phoneMatch ? parts.indexOf(phoneMatch) : undefined);
...
export function handleAuthError(error, navigate // Pass navigate as parameter
) {
    console.log(error);
...
export function getRouteStatusText(status, date) {
    const now = moment();
